...
### 🛠️ 따라하기 캡스톤 — 처음부터 직접 만들기

- 🏗️ [**빌드 튜토리얼**](./build-tutorial.md) — `create-next-app`부터 완성까지 **12개 마일스톤 TODO**로
  우리 게임의 **모든 웹기술**을 당신 손으로 다시 구현한다. **복붙 금지** — 각 단계가 우리 문서·코드의
  "찾아볼 곳"만 알려주고, 읽고 이해해 직접 짠다. (5일 커리큘럼을 끝낸 뒤 캡스톤으로 권장)
