  ...
  return (
    <div className="anim-fadeup relative z-10 flex h-full flex-col px-6 pb-8 pt-8">
      <h1 className="text-center text-lg font-bold">
        {state.pet.name}
        <span className="ml-2 text-sm font-normal text-white/50">지구 육성 중</span>
      </h1>

      <div className="mt-4 flex flex-col gap-2">
        <Gauge label="유대감" value={state.bond} color="#f9a8d4" />
      ...
      <p className="mb-3 text-center text-xs text-white/50">
        {bondDone
          ? "충분한 유대감이 쌓였어요. 이제 함께 우주로!"
          : "탭해서 쓰다듬고, 우주젤리로 마음을 나눠보세요"}
      </p>
      ...
      {bondDone ? (
        ...
        >
          {feedRemainMs > 0
            ? `우주젤리 준비 중… ${Math.ceil(feedRemainMs / 1000)}초`
            : "우주젤리 주기 🍮"}
        </button>
      )}
    </div>
