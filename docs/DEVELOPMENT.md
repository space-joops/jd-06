...
우주쓰레기를 먹는 생명체 "아스트로펫"을 분양받아 지구에서 유대감을 쌓고, 우주로 발사한 뒤 궤도(3분)마다 상공을 지날 때 열리는 재회 윈도우에 교감하며 우주를 정화한다.

브레인스토밍으로 확정된 4대 방향 (변경하려면 사용자와 합의 필요):

1. **감성 돌봄 중심** — 숫자 성장이 아니라 교감이 핵심. 죽음/처벌 금지. 방치하면 "시무룩"해질 뿐이고, 복귀하면 반겨준다. 죄책감이 아니라 애틋함을 주는 디자인.
2. **압축 시간** — 궤도 한 바퀴 3분. 재회 윈도우는 우리 집 상공(정점)을 가운데로 좌우 45°(궤도의 25%).
...
- **orbitInfo(launchedAt, now)**: `phase = (경과 % 3분) / 3분` (0 = 상공 정점). 상공 기준
  부호거리 `off = phase>0.5 ? phase-1 : phase`, `|off| ≤ REUNION_HALF(=45/360)`이면 재회 윈도우.
  즉 상공을 **가운데**로 좌우 45°. 발사 직후 phase 0 = 상공 한가운데 → 즉시 재회 상태(첫 재회 연출).
- **windowIndex** = `round(경과/궤도)` = 상공 통과 기준 "재회 패스" 번호. 윈도우가 phase 0
  경계를 걸쳐 두 `index`에 나뉘어도 한 재회는 하나의 windowIndex로 묶인다.
- 재회 패스당 1회 액션(간식·협동 수거)은 `lastSnackOrbit / lastCoopOrbit`에 **windowIndex**를
  기록해 제한 (필드명은 그대로, 값 의미만 "재회 패스 번호"로 확장 — 세이브 스키마 불변).
...
| 게임 가스/추력 | GAS_MAX 70(env `NEXT_PUBLIC_GAS_MAX`), THRUST 900, GAS_BURN 11/s, 위성 충전 +28, 위험물 −20 | `constants.ts` |
| 위성 등장 | 9초 간격+화면에 없을 때만(한 종류씩), 30% 스타링크 트레인 | `SAT_SPAWN_MS` |
| 간식 | 기분 +15 | 재회 패스당 1회 |
